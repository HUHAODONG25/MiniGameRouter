...
void from_svc_thread(Helper& helper, std::string svc_name) {
    std::string input;
    while (running) {
...
int main(int argc, char* argv[]) {
    ...
    // 输入目的地址
    std::string recv_svc_name;

    std::cin >> recv_svc_name;
    
    // 创建并启动两个线程
    std::thread input([&helper, recv_svc_name]() {
        from_svc_thread(helper, recv_svc_name);
    });
